...
function renderEventList() {
    ...
    for (const e of filtered) {
        const d = new Date(e.date || e.day);
        html += `<tr><td>${d.getDate().toString().padStart(2,'0')}.${(d.getMonth()+1).toString().padStart(2,'0')}.${d.getFullYear()}</td><td><a href="${e.organizer_url}" target="_blank">${e.event ? e.event.split(' - ')[0] : ''}</a></td><td>${e.track}</td></tr>`;    }
    html += '</tbody></table>';
