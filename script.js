...
function renderEventList() {
    ...
    for (const e of filtered) {
        const d = new Date(e.date || e.day);
        html += `<tr><td>${d.getDate().toString().padStart(2,'0')}.${(d.getMonth()+1).toString().padStart(2,'0')}.${d.getFullYear()}</td><td>${e.event ? e.event.split(' - ')[0] : ''}</td><td>${e.track}</td><td>${e.flag}</td><td><a href="${e.website}" target="_blank">${translations[currentLang]?.officialWebsite||'Site'}</a></td></tr>`;    }
    html += '</tbody></table>';
