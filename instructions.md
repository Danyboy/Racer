...
8 Сделай скипт на питоне котоый будет ходить по списку из ссылок из links.json и брать календари, отправлять их в deepseek который будет формировать json мероприятий - трек, день, цена если есть, ссылка на организатора - очень важно ставить правильные айди для треков, при запросе в дипсик передавай айди треков из track.json. Потом объединять все запросы и результирующией json в один и сохранять его в events.json и бекапить старый файл с именем с датой и временем 

9 @script.js @index.html На виде с листом отфильтруй уже проешдшие события чтобы их не было видно
